...
function StatCard({ label, value, accent = '#60a5fa', sub }) {
  return (
    <div style={{
      background: '#0d1e38',
      ...
      <span style={{ color: '#64748b', fontFamily: 'monospace', fontSize: 10, letterSpacing: 1 }}>
        {label.toUpperCase()}
      </span>
...
function TerminalCard({ terminal, flights }) {
  ...
  const accent = isPax ? '#60a5fa' : '#4ade80'
  return (
    <div style={{
      background: '#0d1e38',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 16px' }}>
        {terminal.gate_count != null && (
          <>
            <span style={{ color: '#64748b', fontSize: 11, fontFamily: 'monospace' }}>GATES</span>
            <span style={{ color: '#e2e8f0', fontSize: 12, textAlign: 'right' }}>{terminal.gate_count}</span>
          </>
        )}
        {terminal.bay_count != null && (
          <>
            <span style={{ color: '#64748b', fontSize: 11, fontFamily: 'monospace' }}>BAYS</span>
            <span style={{ color: '#e2e8f0', fontSize: 12, textAlign: 'right' }}>{terminal.bay_count}</span>
          </>
        )}
        {flights != null && (
          <>
            <span style={{ color: '#64748b', fontSize: 11, fontFamily: 'monospace' }}>ACTIVE FLIGHTS</span>
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
        <span style={{ color: '#94a3b8', fontFamily: 'monospace', fontSize: 11, letterSpacing: 1 }}>
          {label}
        </span>
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10 }}>
        <StatCard label="Free Gates"   value={freeGates}    accent="#60a5fa" />
        <StatCard label="Total Flights" value={totalFlights} accent="#e2e8f0" />
        <StatCard label="Inbound"      value={inboundCount} accent="#4ade80" />
        <StatCard label="Delayed"      value={delayedCount} accent="#f87171" />
      </div>
      ...
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          {terminals.map(t => (
            <TerminalCard key={t.terminal_id} terminal={t} />
          ))}
          {terminals.length === 0 && (
            <div style={{ color: '#334155', fontFamily: 'monospace', fontSize: 11 }}>No terminal data</div>
          )}
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{ color: '#64748b', fontFamily: 'monospace', fontSize: 10, letterSpacing: 1.5, marginBottom: 14 }}>
          RUNWAY UTILIZATION
        </div>
        {runways.length > 0 ? (
          runways.map((rwy, i) => (
      ...
      {flightCounts.length > 0 && (
        <div style={{
          ...
          <div style={{ color: '#64748b', fontFamily: 'monospace', fontSize: 10, letterSpacing: 1.5, marginBottom: 12 }}>
            FLIGHT STATUS BREAKDOWN
          </div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            {flightCounts.map(fc => (
              <div key={fc.status} style={{
                ...
                <span style={{ color: '#e2e8f0', fontFamily: 'monospace', fontSize: 13, fontWeight: 700 }}>
                  {fc.count}
                </span>
              </div>
            ))}
